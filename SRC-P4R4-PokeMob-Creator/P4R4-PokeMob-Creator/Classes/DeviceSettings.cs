...
namespace P4R4_PokeMob_Creator
{
    ...
    /// <summary>
    /// THANKS TO THE DEVS AT POKEMOBBOT FOR THEIR RANDOM DEVICE CLASS USED HERE
    /// </summary>
    public class DeviceSettings
    {
        public IDictionary<string, string> phone_item;
        ...
        public DeviceSettings()
        {
            ...
            DeviceId = RandomString(16, "0123456789abcdef"); // "ro.build.id";
            AndroidBoardName = phone_item["board"]; // "ro.product.board";
            AndroidBootLoader = "unknown"; //"ro.product.bootloader; //I think
            DeviceBrand = phone_item["mft"];// "product.brand";
            DeviceModel = phone_item["model"]; //"product.device";
            DeviceModelIdentifier = phone_item["name"] + "_" + RandomString(random.Next(4, 10), "0123456789abcdef");// "build.display.id";
            DeviceModelBoot = phone_item["board"]; //"boot.hardware";
            HardwareManufacturer = phone_item["mft"]; //"product.manufacturer";
            HardWareModel = phone_item["model"]; //"product.model";
            FirmwareBrand = phone_item["board"]; //"product.name"; //iOS is "iPhone OS"
